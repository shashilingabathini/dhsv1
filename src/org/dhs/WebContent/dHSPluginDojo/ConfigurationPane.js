define([
	...
	function(declare, _TemplatedMixin, _WidgetsInTemplateMixin, PluginConfigurationPane, dojoJson,template) {
		...
		load: function(callback) {
			...
			if (this.configurationString) {
				try {
					...
					this._ignoreAdminUsers.set('value',jsonConfig.configuration[5].value)
				} catch (e) {
		...
		_onFieldChange : function() {
			...
			configString = {  
				name: "_ignoreAdminUsers",
				value: this._ignoreAdminUsers.get('value')
			}; 
			configArray.push(configString);
